...
function setControl(instance) {
  ...
  mapInstance$5 = instance;
  var _options$control = options.control,
      addAttributionConfig = _options$control.addAttribution,
      zoomConfig = _options$control.zoom,
      scaleConfig = _options$control.scale; // 水印信息加载
...
function addAttributionControl(options) {
  ...
  var attribution = mapInstance$5.attributionControl;
...
function addZoomControl(options) {
  ...
  mapInstance$5.addControl(zoomControl);
}
/**
 * 比例尺控件
 */
...
function addScaleControl(options) {
  ...
  mapInstance$5.addControl(scaleControl);
}
...
function createMap() {
  ...
  setControl(mapInstance$4, options);
...
function initialize$3(instance, options) {
  mapInstance$3 = instance;
  layerGroup$3 = new L.FeatureGroup();
  mapInstance$3.addLayer(layerGroup$3);
  ...
  mapInstance$3.off(L.Draw.Event.CREATED).on(L.Draw.Event.CREATED, function (event) {
    var drawlayer = event.layer;
    layerGroup$3.addLayer(drawlayer);

    if (pointLayerGroup$3) {
      setTimeout(function () {
        drawEndPoint();
        mapInstance$3.addLayer(pointLayerGroup$3);
      }, 0);
    }
  });
...
function drawEndPoint() {
  ...
  if (pointLayerGroup$3) {
    pointLayerGroup$3.eachLayer(function (e) {
      i += 1;
      lines.push(e._latlng);

      if (i === Object.keys(pointLayerGroup$3._layers).length) {
        e.setTooltipContent(buildHtml$3("\u5171".concat(formatLength(lines), "<div onclick=\"polyline.deleteLine(").concat(e._leaflet_id, ")\">\u5220\u9664</div>")));
      }
    });
  }
}
...
var pointLayerGroup$2;
var layerGroup$2;
var drawControl$2;

function initialize$2(instance, options) {
  mapInstance$2 = instance;
  layerGroup$2 = new L.FeatureGroup();
  pointLayerGroup$2 = new L.FeatureGroup();
  mapInstance$2.addLayer(layerGroup$2);
  mapInstance$2.addLayer(pointLayerGroup$2);
  ...
  mapInstance$2.off(L.Draw.Event.CREATED).on(L.Draw.Event.CREATED, function (event) {
    var drawlayer = event.layer;
    var latlng = drawlayer.getLatLngs()[0];
    layerGroup$2.addLayer(drawlayer);
...
function addMeasureMarker$2(latlng) {
  ...
  marker.bindTooltip(buildHtml$2("".concat(formatArea$2(latlng), "<div onclick=\"polygon.deleteLine()\">\u5220\u9664</div>")), {
    permanent: true,
    direction: "right",
    className: "measure-tooltip"
  }).openTooltip();
  pointLayerGroup$2.addLayer(marker);
}

window.polygon = {};

window.polygon.deleteLine = function (e) {
  pointLayerGroup$2.remove();
  layerGroup$2.remove();
};
...
var pointLayerGroup$1;
var layerGroup$1;
var drawControl$1;

function initialize$1(instance, options) {
  mapInstance$1 = instance;
  layerGroup$1 = new L.FeatureGroup();
  pointLayerGroup$1 = new L.FeatureGroup();
  mapInstance$1.addLayer(layerGroup$1);
  mapInstance$1.addLayer(pointLayerGroup$1);
  ...
  mapInstance$1.off(L.Draw.Event.CREATED).on(L.Draw.Event.CREATED, function (event) {
    var drawlayer = event.layer;
    layerGroup$1.addLayer(drawlayer);
    addMeasureMarker$1(drawlayer._mRadius, drawlayer._latlng);
  });
...
function addMeasureMarker$1(radius, latlng) {
  ...
  marker.bindTooltip(buildHtml$1("".concat(formatArea$1(radius), "<div onclick=\"circle.deleteLine()\">\u5220\u9664</div>")), {
    permanent: true,
    direction: "right",
    className: "measure-tooltip"
  }).openTooltip();
  pointLayerGroup$1.addLayer(marker);
}

window.circle = {};

window.circle.deleteLine = function (e) {
  pointLayerGroup$1.remove();
  layerGroup$1.remove();
};
...
function formatArea$1(radius) {
  var area = (radius / 1000).toFixed(2) + "k㎡";
  return area;
}
...
function addMeasureMarker(latlng, center) {
  ...
  marker.bindTooltip(buildHtml("".concat(formatArea(latlng), "<div onclick=\"rectangle.deleteLine()\">\u5220\u9664</div>")), {
    permanent: true,
    direction: "right",
    className: "measure-tooltip"
  }).openTooltip();
