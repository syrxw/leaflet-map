  ...
  if (point) {
    point.eachLayer(function (e) {
      ...
      if (i === Object.keys(point._layers).length) {
        e.setTooltipContent(buildHtml$3("共" + formatLength(lines) + "<div onclick='polyline.deleteLine(\"" + currentLayerId$3 + "\")'>删除</div>"));
      }
    });
  }
...
function addMeasureMarker$2(latlng) {
  ...
  marker.bindTooltip(buildHtml$2("".concat(formatArea$2(latlng), "<div onclick=\"polygon.deleteLine('").concat(currentLayerId$2, "')\">\u5220\u9664</div>")), {
    permanent: true,
    direction: "right",
    className: "measure-tooltip"
  }).openTooltip();
...
function initialize$1(instance, options) {
  ...
  mapInstance$1.off(L.Draw.Event.CREATED).on(L.Draw.Event.CREATED, function (event) {
    ...
    addMeasureMarker$1(event.layer._mRadius, event.layer._latlng);
...
function addMeasureMarker$1(radius, latlng) {
  ...
  marker.bindTooltip(buildHtml$1("".concat(formatArea$1(radius), "<div onclick=\"circle.deleteLine('").concat(currentLayerId$1, "')\">\u5220\u9664</div>")), {
    permanent: true,
    direction: "right",
    className: "measure-tooltip"
  }).openTooltip();
...
function addMeasureMarker(latlng, center) {
  ...
  marker.bindTooltip(buildHtml("".concat(formatArea(latlng), "<div onclick=\"rectangle.deleteLine('").concat(currentLayerId, "')\">\u5220\u9664</div>")), {
    permanent: true,
    direction: "right",
    className: "measure-tooltip"
  }).openTooltip();
