window.L.Mask = window.L.LayerGroup.extend({
  options: {
    color: "#3388FF",
    weight: 2,
    fillColor: "#FFFFFF",
  ...
  initialize: function (geojson, options, map) {
  ...
  updateBounds: function (coords) {
    for (var i = 0, len = coords.length; i < len; i++) {
      ...
      for (var j = 0, lenJ = coords2.length; j < lenJ; j++) {
        this._bounds.extend(new window.L.latLng(coords2[1], coords2[0], coords2[2]));
      }
    }
  },
  addMaskLayer: function () {
    var latlngs = this.coordsToLatLngs(this._maskPolygonCoords);
    var layer = new window.L.polygon(latlngs, this.options);
    this.addLayer(layer);
...
window.L.mask = function (geojson, options, map) {
  return new window.L.Mask(geojson, options, map);
};
