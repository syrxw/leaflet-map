...
L.Control.WindPosition = L.Control.extend({
	...
	_onMouseMove: function _onMouseMove(e) {
		...
		if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
			...
			htmlOut = "<strong>风向: </strong>" + self.vectorToDegrees(gridValue[0], vMs) + "°" + ", <strong>风速: </strong>" + self.vectorToSpeed(gridValue[0], vMs).toFixed(1) + "m/s" + ", <strong>温度: </strong>" + (gridValue[2] - 273.15).toFixed(1) + "°C";
		} else {
...
(function (root, factory) {
	...
	var WindJSLeaflet = {
		...
		init: function init(options) {
			...
			WindJSLeaflet._checkWind(options).then(function () {
				...
				WindJSLeaflet._canvasLayer.addTo(WindJSLeaflet._map);
   ...
   */
		...
		_loadLocalData: function _loadLocalData() {
			...
			/*$.getJSON('../preData/demo-fengxiang.json', function (data) {
				WindJSLeaflet._data = data;
				WindJSLeaflet._initWindy(data);
			});*/
			
			// WindJSLeaflet._data = data;
			WindJSLeaflet._initWindy(this._options.defaultData);
		},
